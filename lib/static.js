...
// Make sure that the HTTPServer class actually uses pipelining
// Set Etags and some kind of If-No-Match header
...
FileServer.prototype.addFile = function(options) {
	...
	if(compress || type.indexOf('text/') == 0) { // Only compress text items for now
		zlib.gzip(data, function(compressed) { // Check this in the event of problems, callbacks may not be official in the docs
			if((compressed.length < data.length - 1024 || compressed.length < data.length * 0.9)) {// If the compression ratio is good enough (at least a KB saved or at least 10%)
				self.staticCache[url].compressed = res.value;
...
FileServer.prototype.handleRequest = function(req, res, next) {
	// Add support for accept-encoding gzip, send Content-Encoding
	// Test and compare the outputs of all the algos in zlib
	
	var u = url.parse(req.url).pathname;
	var h, t = this.staticCache[u], data, headers = {'Content-Type': t.type, 'ETag': t.etag}, not_modified = false;
